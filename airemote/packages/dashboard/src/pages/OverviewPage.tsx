...
function DeviceCard({ device }: { device: Device }) {
  ...
  return (
    <Link
      ...
      className={clsx(
        'block glass glass-hover rounded-xl p-4 cursor-pointer transition-all animate-fade-in',
        isOnline ? 'border-emerald-500/10' : ''
      )}
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-medium text-sm text-slate-100">{device.name}</h3>
            <p className="text-xs text-slate-500 mt-0.5">{device.info?.hostname || device.id.slice(0, 8)}</p>
          </div>
        </div>
        <span className={clsx(
            ...
            {[
              ...
              return (
                <div key={item.label}>
                  ...
                  <div className="text-sm font-mono font-medium text-slate-200">{item.value}%</div>
      ...
      {!isOnline && device.lastSeen && (
        <p className="text-xs text-slate-600 mt-1">
          آخر اتصال: {new Date(device.lastSeen).toLocaleString('ar')}
        </p>
      )}
    </Link>
  )
...
export function OverviewPage() {
  ...
  // Aggregate stats across online devices
  const onlineDevices = devices.filter(d => d.status === 'online')
  const avgCpu = onlineDevices.length > 0
    ? Math.round(onlineDevices.reduce((sum, d) => sum + (statsMap[d.id]?.cpuPercent || 0), 0) / onlineDevices.length)
    : 0
  const avgRam = onlineDevices.length > 0
    ? Math.round(onlineDevices.reduce((sum, d) => sum + (statsMap[d.id]?.ramPercent || 0), 0) / onlineDevices.length)
    : 0
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">حالة جميع الأجهزة في الوقت الفعلي</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-slate-400">متصل</span>
          </div>
          <div className="text-3xl font-bold text-emerald-400">{online}</div>
          <p className="text-xs text-slate-600 mt-1">من أصل {devices.length} جهاز</p>
        </div>
        <div className="glass rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-xs text-slate-600 mt-1">جهاز غير نشط</p>
        </div>
        <div className="glass rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <AlertCircle size={15} className="text-orange-400" />
            <span className="text-sm text-slate-400">تنبيهات</span>
          </div>
          <div className="text-3xl font-bold text-orange-400">{alerts}</div>
          <p className="text-xs text-slate-600 mt-1">موارد عالية الاستهلاك</p>
        </div>
        <div className="glass rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp size={15} className="text-brand-blue" />
            <span className="text-sm text-slate-400">متوسط CPU</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/devices" className="text-xs text-brand-blue hover:underline">عرض الكل</Link>
          </div>
          ...
          {!loading && devices.length === 0 && (
            <div className="glass rounded-xl text-center py-14">
              ...
              <h3 className="text-slate-300 font-medium mb-1">لا توجد أجهزة بعد</h3>
              <p className="text-slate-500 text-sm mb-4">أضف جهازاً وثبّت عليه الـ Agent للبدء</p>
              <Link
                to="/devices"
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/sessions" className="text-xs text-brand-blue hover:underline">عرض الكل</Link>
          </div>
          ...
          <div className="glass rounded-xl overflow-hidden">
            ...
            {!sessionsLoading && sessions.length === 0 && (
              <div className="text-center py-8 text-xs text-slate-600">لا توجد جلسات بعد</div>
            )}
            {!sessionsLoading && sessions.map((s, i) => {
              const typeCfg = SESSION_TYPE_LABELS[s.type] || { label: s.type.toUpperCase(), color: 'text-slate-400 bg-slate-700/50' }
              ...
              return (
                <div key={s.id} className={clsx('flex items-center gap-3 px-4 py-2.5', i < sessions.length - 1 && 'border-b border-slate-700/30')}>
                  <div className="flex-shrink-0">
                    {isActive
                      ? <Circle size={7} className="text-emerald-400 fill-current" />
                      : <Circle size={7} className="text-slate-600 fill-current" />
                    }
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-slate-200 truncate">
                      {s.deviceName || s.deviceId?.slice(0, 8)}
                    </p>
                    <p className="text-[10px] text-slate-500">
                      {new Date(s.startedAt).toLocaleString('ar', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    <span className={clsx('text-[10px] px-1.5 py-0.5 rounded font-medium', typeCfg.color)}>
                      {typeCfg.label}
                    </span>
                    <span className="text-[10px] text-slate-600 font-mono">
                      {isActive ? <span className="text-emerald-400">نشطة</span> : formatDuration(s.durationSec)}
                    </span>
                  </div>
                </div>
              )
