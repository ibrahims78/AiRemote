...
async function importBackup(zipPath, logger) {
  ...
  return new Promise((resolve, reject) => {
    ...
    execFile(cmd, args, { timeout: 3e4 }, (err) => {
      if (err) {
        execFile("7z", ["x", zipPath, `-o${tmpDir}`, "-y"], { timeout: 3e4 }, (err2) => {
...
function doRestore(tmpDir, resolve, reject, logger) {
  try {
    ...
    if (fs.existsSync(metaSrc)) {
      try {
        meta = JSON.parse(fs.readFileSync(metaSrc, "utf8"));
      } catch {
      }
    }
    if (fs.existsSync(dbSrc)) {
      fs.copyFileSync(dbSrc, dbDest);
      logger?.info("backup", "Restored airemote.db");
    }
...
function scheduleBackup(config, logger) {
  ...
  const ms = config.intervalHours * 60 * 60 * 1e3;
